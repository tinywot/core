...
tinywot_status_t tinywot_thing_dynamic_set_form(
  ...
  size_t form_i = 0;
  struct tinywot_form *form_i_p = self->forms;
...
tinywot_status_t tinywot_thing_base_process_request(
  ...
  status = tinywot_thing_base_get_form(
    self, request->target, request->operation_type, &form
  );
  ...
  switch (status) {
    ...
    default:
      response->status = TINYWOT_RESPONSE_STATUS_INTERNAL_ERROR;
      break;
  }
