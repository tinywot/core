...
/*!
  \file
  \brief Unit tests for `tinywot_payload_append()`.
*/
...
static unsigned char const lipsum[] =
  {'L', 'o', 'r', 'e', 'm', ' ', 'i', 'p', 's', 'u', 'm'};
static unsigned char const dsamet[] =
  {' ', 'd', 'o', 'l', 'o', 'r', ' ', 's', 'i', 't', ' ', 'a', 'm', 'e', 't'};
static unsigned char const lidsamet[] = {
  'L', 'o', 'r', 'e', 'm',
  ' ', 'i', 'p', 's', 'u', 'm',
  ' ', 'd', 'o', 'l', 'o', 'r',
  ' ', 's', 'i', 't',
...
static void tinywot_payload_append_should_succeed(void) {
  ...
  /* ++ 'Lorem ipsum' */
  sc = tinywot_payload_append(pl, lipsum, sizeof(lipsum));
  TEST_ASSERT_EQUAL(sc, TINYWOT_STATUS_SUCCESS);
  TEST_ASSERT_EQUAL_MEMORY(lipsum, pl->content, sizeof(lipsum));
  TEST_ASSERT_EQUAL_UINT(
    TINYWOT_TEST_MEMORY_SIZE_BYTE, pl->content_buffer_size_byte
  );
  ...
  /* ++ ' dolor sit amet' */
  sc = tinywot_payload_append(pl, dsamet, sizeof(dsamet));
  TEST_ASSERT_EQUAL(sc, TINYWOT_STATUS_SUCCESS);
  TEST_ASSERT_EQUAL_MEMORY(lidsamet, pl->content, sizeof(lidsamet));
  TEST_ASSERT_EQUAL_UINT(
    TINYWOT_TEST_MEMORY_SIZE_BYTE, pl->content_buffer_size_byte
  );
